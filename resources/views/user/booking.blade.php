...
<body>
    ...
    <main class="booking-page">
        <section class="page-header-block position-relative d-flex align-items-center justify-content-center"
            style="background-image:url({{ asset('userside_assets/assets/images/about-img.jpeg') }});">
            <div class="text-center page-header-content mt-5">
                <h1 class="position-relative text-white fs-1 fw-bold text-uppercase">Your Bookings</h1>
                <p class="text-white fs-6 fw-normal">All your beach bookings are listed below.
                    You can cancel, or view details for any reservation directly from this panel..</p>
            </div>
        </section>
        @if (!empty($bookings))
                            ...
                            <div class="col-12 bg-white rounded-3 shadow mb-3">
                                <div class="row align-items-center">
                                    ...
                                    <div class="col-md-8 p-3">
                                        ...
                                        <p class="card-description fw-normal lh-sm mb-4 text-black">Your reservation is scheduled from {{ $booking->booking_date }} until {{ $booking->end_booking_date }}.
To cancel your booking, simply click the Cancel button.</p>
                                        <div class="d-flex gap-2">
                                            {{-- <a href="#" class="btn btn-outline-primary">Edit</a> --}}
                                            <a href="#" class="btn btn-outline-danger" onclick="cancelReservation('{{$booking->id}}')">Cancel</a>
                                        </div>
                                    </div>
                                </div>
                            </div>
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: `/cancel/user/reservation/${reservationId}`,
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}'
                },
                ...
                success: function(response) {
                    ...
                    if (response.status === 'success') { // ✅ check "status"
                        Swal.fire({
                            icon: 'success',
                            title: 'Reservation Cancelled',
                            text: response.message ?? 'Your reservation has been cancelled successfully.',
                        ...
                        Swal.fire({
                            ...
                            text: response.message ?? 'Unable to cancel your reservation. Please try again.'
                        });
                    }
                },
                error: function(xhr) {
                    ...
                    let msg = 'An error occurred while cancelling. Please try again.';
